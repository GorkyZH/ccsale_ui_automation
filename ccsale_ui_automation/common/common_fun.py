...
class Common(BaseView):
    ...
    """ 获取页面元素定位最新方法"""
    def get_by_loc(self, page, key, index=0):
        yaml_data = self.parse_yaml('page')
        ...
        if type == 'text':
            ...
            by_loc = 'new UiSelector().resourceId("%s").childSelector(new UiSelector().index(%d))' % (loc, index)
        else:
            raise Exception("type is illegal, type: " +type)
        return by_loc

    def get_by_loc_index(self, page, key, index):
        yaml_data = self.parse_yaml('page')
        loc = yaml_data[page]['locators'][key]['loc']
        type = yaml_data[page]['locators'][key]['type']
        if type == 'id_index':
            # by_loc = 'new UiSelector().resourceId("'+loc+'")'+'['+str(index)+']'
            by_loc = 'new UiSelector().resourceId("%s").childSelector(new UiSelector().index(%d))' % (loc, index)
        else:
            raise Exception("type is illegal, type: " + type)
        return by_loc
