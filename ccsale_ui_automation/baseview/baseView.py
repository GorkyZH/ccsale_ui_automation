...
class BaseView(object):
    ...
    def getTextarry(self, loc, index):
        return self.find_elements_by_uiautomator(loc)[index].text

    def click(self, loc):
        self.find_element_by_uiautomator(loc).click()

    def click_index(self, loc, index=0):
    ...
    def enableAppiumUnicodeIME(self):
        command = 'adb shell ime set io.appium.android.ime/.UnicodeIME'
        os.system(command)

    # 切换搜狗输入法为当前输入法
    def enableSougouIME(self):
